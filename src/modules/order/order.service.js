...
class OrderService {
  ...
  async createOrder(payload, user) {
    const { senderId, senderAddressId, courierId, products, receivers } =
      payload;
    ...
    if (!senderAddressDetails) {
      const error = new Error(
        "Invalid senderAddressId: Address not found for sender.",
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    const hasReceivers =
      Array.isArray(resolvedReceivers) && resolvedReceivers.length > 0;
  ...
  async _resolveProducts(products) {
    ...
    for (const p of products) {
      ...
      if (!variation) {
        const error = new Error(
          `Invalid variationId: ${p.variationId} not found.`,
        );
        error.statusCode = 400;
        throw error;
      }
      ...
      if (!unitId && variation.FkProductId) {
        const product = await orderRepository.resolveProduct(
          variation.FkProductId,
        );
      ...
      resolved.push({
        ...
        colorId: variation.FkLuColorId || variation.fkLuColorId || null,
        qty: p.quantity,
  ...
  async _resolveReceivers(receivers) {
    ...
    for (const r of receivers) {
      ...
      if (!party) {
        const error = new Error(
          `Invalid receiverId: ${r.receiverId} not found.`,
        );
        error.statusCode = 400;
        throw error;
      }

      const address = await orderRepository.resolveAddress(
        r.receiverId,
        r.receiverAddressId,
      );
      if (!address) {
        const error = new Error(
          `Invalid receiverAddressId: ${r.receiverAddressId} not found for receiver.`,
        );
        error.statusCode = 400;
        throw error;
      }
  ...
  async _createOrderLive(ctx) {
    ...
    const fullOrder = await orderRepository.findById(
  ...
  async _createOrderMock(ctx) {
    ...
    const aggregatedReceivers = await this._processMockReceivers(
      order.id,
      receiversList,
      ctx.courierId,
    );
  ...
  async _processMockReceivers(orderId, list, courierId) {
    ...
    for (const rec of list) {
      ...
      const parcel = await orderRepository.createParcel(
        recRecord.id,
        courierId,
      );
  ...
  _calculateTotalAmount(receivers) {
    return receivers.reduce((total, rec) => {
      return (
        total +
        (rec.products || []).reduce(
          (sub, p) => sub + (p.unitPrice || 0) * (p.qty || 0),
          0,
        )
      );
    }, 0);
  ...
  async updateOrder(orderId, payload, user) {
    ...
    const existingReceivers =
      await orderRepository.getReceiversForOrder(orderId);
    const existingReceiverIds = existingReceivers.map(
      (r) => r.PkReceiverDetailsId || r.id,
    );
    const existingItems =
      await orderRepository.getItemsForReceivers(existingReceiverIds);
    const existingParcels =
      await orderRepository.getParcelsForReceivers(existingReceiverIds);
    ...
    if (payload.senderId && payload.senderAddressId) {
      ...
      headerPayload = {
        ...
        senderAddress:
          address.Address || address.addressLine1 || address.address,
    ...
    if (payload.receivers && Array.isArray(payload.receivers)) {
      ...
      const resolvedReceivers = await this._resolveUpdateReceivers(
        payload.receivers,
      );

      // Compute diffs
      diffs.receivers = this._diffReceivers(
        existingReceivers,
        resolvedReceivers,
        existingItems,
        existingParcels,
      ...
      headerPayload.totalAmount = this._calculateTotalAmount([
        ...diffs.receivers.toUpdate,
        ...diffs.receivers.toCreate,
      ]);
    } else {
      ...
      const currentResolved = existingReceivers.map((r) => {
        ...
        return {
          ...
          products: existingItems
            .filter(
              (i) => (i.FkReceiverDetailsId || i.fkReceiverDetailsId) === rId,
            )
            .map((i) => ({
    ...
    await orderRepository.updateOrderGraph(
      orderId,
      headerPayload,
      diffs,
      adminId,
  ...
  async _resolveUpdateReceivers(receivers) {
    ...
    for (const rec of receivers) {
      ...
      const address = await orderRepository.resolveAddress(
        rec.receiverId,
        rec.receiverAddressId,
      );

      if (!party || !address) {
        const error = new Error(
          `Invalid receiverId or receiverAddressId for receiver ${rec.receiverId}`,
        );
        error.statusCode = 400;
        throw error;
      }
  ...
  _diffReceivers(existing, incoming, allExistingItems, allExistingParcels) {
    ...
    for (const inc of incoming) {
      if (inc.receiverDetailsId) {
        ...
        const exItems = allExistingItems.filter(
          (i) =>
            (i.FkReceiverDetailsId || i.fkReceiverDetailsId) ===
            inc.receiverDetailsId,
        );
    ...
    for (const ex of existing) {
      ...
      if (!incomingMap.has(exId)) {
        const parcel = allExistingParcels.find(
          (p) => (p.FkReceiverDetailsId || p.fkReceiverDetailsId) === exId,
        );
        const exItems = allExistingItems.filter(
          (i) => (i.FkReceiverDetailsId || i.fkReceiverDetailsId) === exId,
        );
        diffs.toRemove.push({
          ...ex,
  ...
  _checkPhysicalExecutionBegun(parcels, action = "update") {
    ...
    if (
      ...
      const error = new Error(
        `Cannot ${action} order: physical execution has begun on one or more parcels.`,
      );
      error.statusCode = 400;
      throw error;
    }
  ...
  _mapOrderSummary(o) {
    ...
    if (!totalParcels && o.receivers && Array.isArray(o.receivers)) {
      totalParcels = o.receivers.filter(
        (r) => r.parcel || r.parcelId || r.PkParcelDetailsId,
      ).length;
    }
    ...
    return {
      ...
      derivedStatus:
        o.DynamicOrderStatus || o.DerivedStatus || o.derivedStatus || "Pending",
      createdAt: o.OrderDate || o.CreatedAt || o.createdAt,
      expectedDeliveryDate:
        o.ExpectedDeliveryDate || o.expectedDeliveryDate || null,
    ...
    return {
      ...
      receivers: (o.receivers || []).map((r) => ({
        ...
        parcel: r.parcel
          ? {
              parcelId:
                r.parcel.PkParcelDetailsId ||
                r.parcel.id ||
                r.parcel.parcelDetailsId,
              parcelCode:
                r.parcel.parcelCode ||
                r.parcel.ParcelCode ||
                ParcelCodeService.generateCode(
