...
class ParcelService {
  ...
  async _mapParcel(parcel) {
    ...
    return {
      ...
      status:
        parcel.ParcelStatusName ||
        parcel.StatusDescription ||
        parcel.status ||
        parcel.parcelStatusCode ||
        STATUS.PENDING,
      labelPrintCount:
        parcel.LabelPrintCount !== undefined
          ? parcel.LabelPrintCount
          : parcel.labelPrintCount,
      ...
      pincode:
        parcel.ReceiverPincode || parcel.Pincode || parcel.pincode || null,
  ...
  async getParcelList(filters) {
    ...
    return {
      data: await Promise.all(data.map((p) => this._mapParcel(p))),
      total,
    };
  }
  ...
  async logLabelPrint(id, user) {
    ...
    const result = await parcelRepository.updateParcelState(
      id,
      1,
      null,
      0,
    ...
    for (const id of parcelDetailsIds) {
      ...
      if (parcel.status !== STATUS.AWB_LINKED) {
        const error = new Error(
          `Parcel ${parcel.parcelId} cannot be dispatched (Status: ${parcel.status})`,
        );
        error.statusCode = 400;
        throw error;
      }
  ...
  async _getAndValidateForLinking(parcelId) {
    ...
    if (parcel.status !== STATUS.LABEL_PRINTED) {
      const error = new Error(
        `Cannot link AWB: parcel is '${parcel.status}', must be '${STATUS.LABEL_PRINTED}'`,
      );
      error.statusCode = 400;
      throw error;
    }
  ...
  async _ensureUniqueAWB(awbNumber) {
    ...
    if (existingAWB) {
      const error = new Error(
        `AWB '${awbNumber}' is already linked to another parcel`,
      );
